...
```Go
// longestPalindrome 函数用于在给定的字符串 s 中找到最长的回文子串，并返回该子串
// 输入参数：
// - s: 一个字符串，在这个字符串中查找最长回文子串
// 返回值：
// - res: 字符串 s 中最长的回文子串
func longestPalindrome(s string) (res string) {
    // 遍历字符串 s 中的每个字符，以每个字符为中心向两边扩展来寻找回文子串
    for i, n := 0, len(s); i < n; i++ {
        // 初始化左右指针 l 和 r，初始时都指向当前字符 i，用于寻找奇数长度的回文子串
        l, r := i, i
        // 以当前字符为中心向两边扩展，只要左右指针所指字符相等且不越界，就继续扩展
        for l >= 0 && r < n && s[l] == s[r] {
            // 左指针向左移动一位
            l--
            // 右指针向右移动一位
        ...
        // 当扩展结束后，此时 l 和 r 所指的范围 (l+1 到 r-1) 就是以当前字符为中心的最长回文子串
        // r - l - 1 就是这个回文子串的长度
        // 如果这个回文子串的长度大于之前记录的最长回文子串 res 的长度
        if r-l-1 > len(res) {
            // 更新 res 为当前找到的更长的回文子串
            res = s[l+1:r]
        }

        // 重新初始化左右指针 l 和 r，l 指向当前字符 i，r 指向当前字符的下一个字符
        // 用于寻找偶数长度的回文子串
        l, r = i, i+1
        // 以当前字符和下一个字符为中心向两边扩展，只要左右指针所指字符相等且不越界，就继续扩展
        for l >= 0 && r < n && s[l] == s[r] {
            // 左指针向左移动一位
            l--
            // 右指针向右移动一位
        ...
        // 当扩展结束后，此时 l 和 r 所指的范围 (l+1 到 r-1) 就是以当前字符和下一个字符为中心的最长回文子串
        // r - l - 1 就是这个回文子串的长度
        // 如果这个回文子串的长度大于之前记录的最长回文子串 res 的长度
        if r-l-1 > len(res) {
            // 更新 res 为当前找到的更长的回文子串
